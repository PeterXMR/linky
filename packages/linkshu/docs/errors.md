...
| `MintUnreachable`        | `mint`, `detail`                     | Network, timeout, 5xx. **Transient.**                                                                                      | Retry later; nothing was persisted that needs recovery.                                                                          |
...
| `PaymentPending`         | `mint`, `quoteId`, `rowId`, `amount` | The melt was sent and the mint has not settled it either way. **Pending**, not failed.                                     | Record the payment as pending; the inputs stay `reserved` in `rowId` and `Melt.resumePending` settles it ([melt.md](./melt.md)). |
